...
sql = '''
    SELECT * FROM ss WHERE username = %s; 
'''
...
# game over if it touches the border
    ...
    while paused:
        ...
        cntr = font_text.render(f'Your score is {score}', True, 'white')
        screen.blit(cntr, (315, 350))
        l = font_text.render(f'Your level is {level}', True, 'white')
        screen.blit(l, (317, 385))
        txt = font_text.render(
            f'Press "CS" to save your current state and press "P" to continue', True, 'white')
