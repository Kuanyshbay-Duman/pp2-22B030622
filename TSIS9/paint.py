...
def setsurf():
    ...
    tr1 = pg.image.load("Week13\TSIS9\TOOLS\Right_triangle.png")
    buttons.blit(tr1, (8, 8))
    tr2 = pg.image.load("Week13\TSIS9\TOOLS\equilateral-triangle.png")
    buttons.blit(tr2, (8, 55))
    pg.draw.rect(buttons, 'black', (58, 10, 32, 32), 2)
    rh = pg.image.load("Week13\TSIS9\TOOLS\Rhombus.png")
...
def right_triangle(screen, cur, end, d, color):
    ...
    if x1 <= x2:
        if y1 < y2:
            pg.draw.polygon(
                screen, color, [(x1, y1), (x1, y1 + dify), (x2, y2)], d)
        else:
            pg.draw.polygon(
                screen, color, [(x1, y1), (x1, y1 - dify), (x2, y2)], d)
    ...
    for event in pg.event.get():
        ...
        if di['right_triangle'] == 1:
            ...
            if event.type == pg.MOUSEBUTTONUP:
                ...
                triangle(cur_color, [last_pos, pos, ((pos[0] - last_pos[0])*cos(pi/3) - (pos[1] - last_pos[1])*sin(
                    pi/3) + last_pos[0], (pos[0] - last_pos[0])*sin(pi/3) + (pos[1] - last_pos[1])*cos(pi/3) + last_pos[1])])
        elif di['rhombus'] == 1:
            if event.type == pg.MOUSEBUTTONDOWN:
                last_pos = pos
            if event.type == pg.MOUSEBUTTONUP:
                d = get_distance(last_pos, pos)
                rhombus(cur_color, [
                        last_pos, (last_pos[0] + d, last_pos[1]), (pos[0] + d, pos[1]), pos])
        elif di['eraser'] == 1:
            if event.type == pg.MOUSEBUTTONDOWN:
                (x, y) = pos
